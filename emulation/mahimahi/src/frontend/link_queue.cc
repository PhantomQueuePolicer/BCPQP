...
void LinkQueue::record_arrival( const uint64_t arrival_time, const size_t pkt_size, const string & contents)
{
    /* log it */
    if ( log_ ) {
        uint16_t src, dst;
        ...
        while ( bytes_left_in_this_delivery > 0 ) {
            if ( not packet_in_transit_bytes_left_ ) {
                if ( packet_queue_->empty() ) {
                    if (packet_queue_->to_string().substr(0,3)=="shp" || packet_queue_->to_string().substr(0,3)=="bcp" || packet_queue_->to_string().substr(0,3)=="pqp" || packet_queue_->to_string().substr(0,3)=="plc" || packet_queue_->to_string().substr(0,3)=="fpl") {
                        QueuedPacket ret = packet_queue_->dequeue();
                        // *log_ << " packet contents " << ret.contents << endl;
                    }
                    break;
                }
                packet_in_transit_ = packet_queue_->dequeue();
                // *log_ << "actually dequeued packet contents " << packet_in_transit_.contents << endl;
