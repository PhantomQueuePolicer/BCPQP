...
void phantom_dequeue(uint64_t now, shaper_struct* shaper){
  ...
  // shaper->tokens = std::min(shaper->bytes_per_ms * 1000, shaper->bytes_per_ms * time_elapsed_ms);
  shaper->tokens = shaper->bytes_per_ms * time_elapsed_ms;
  // std::cout<<"dequeuing: "<<shaper->tokens<<std::endl;
  ...
  if (shaper->queue_sizes[0] > shaper->tokens/100){
    shaper->queue_sizes[0] -= shaper->tokens/100;
    shaper->dequeues_in_last_period[0] += shaper->tokens/100;
    shaper->tokens -= shaper->tokens/100;
  }
  while (active_queues && sum_qi && shaper->tokens / sum_qi){
    uint64_t per_queue_tokens = shaper->tokens / sum_qi;
    for (uint16_t i=0; i<active_queues; i++){
      uint64_t q_i = shaper->active_queues[i];
      uint64_t q_i_share = q_i * per_queue_tokens;
      // std::cout<<"before "<< shaper->queue_sizes[q_i]<<std::endl;
      if (shaper->queue_sizes[q_i] > q_i_share){
        shaper->queue_sizes[q_i] -= q_i_share;
        shaper->tokens -= q_i_share;
        shaper->dequeues_in_last_period[q_i] += q_i_share;
      } else {
      ...
      if (juggler::time::cycles_to_ms(now - shaper->last_enqueue_calc_time[q_i]) > 200){
        if (active_queues){ 
          shaper->upper_threshold = (shaper->bytes_per_ms * 200.0) / sum_qi;
          shaper->upper_threshold *= q_i;
        } else {
          shaper->upper_threshold = (shaper->bytes_per_ms * 200.0);
        }
        if (shaper->bytes_in_last_period[q_i] < (shaper->upper_threshold * 0.5)){
          // LOG(INFO)<< "In the dequeue" << q_i << " " << juggler::time::cycles_to_ms(now - shaper->last_enqueue_calc_time[q_i]) <<" empty the queue "<< shaper->bytes_in_last_period[q_i] << " " << shaper->upper_threshold * 0.5;
      ...
      // std::cout<<"after "<< shaper->queue_sizes[q_i]<<std::endl;
    }
  }

  if (shaper->tokens){
    if (shaper->queue_sizes[0] > shaper->tokens){
      shaper->queue_sizes[0] -= shaper->tokens;
      shaper->dequeues_in_last_period[0] += shaper->tokens;
      shaper->tokens -= shaper->tokens;
    } else {
      shaper->tokens -= shaper->queue_sizes[0];
      shaper->dequeues_in_last_period[0] += shaper->queue_sizes[0];
      shaper->queue_sizes[0] = 0;
      shaper->active_queues.erase(std::remove(shaper->active_queues.begin(), shaper->active_queues.end(), 0), shaper->active_queues.end());
    }
    if (juggler::time::cycles_to_ms(now - shaper->last_enqueue_calc_time[0]) > 200){
        shaper->upper_threshold = (shaper->bytes_per_ms * 200.0);
        if (shaper->bytes_in_last_period[0] < (shaper->upper_threshold * 0.5)){
          // LOG(INFO)<< "In the dequeue" << q_i << " " << juggler::time::cycles_to_ms(now - shaper->last_enqueue_calc_time[q_i]) <<" empty the queue "<< shaper->bytes_in_last_period[q_i] << " " << shaper->upper_threshold * 0.5;
          shaper->queue_sizes[0] -= std::min(shaper->queue_sizes[0], shaper->magic_packets[0]);
          if (!shaper->queue_sizes[0]){
            shaper->active_queues.erase(std::remove(shaper->active_queues.begin(), shaper->active_queues.end(), 0), shaper->active_queues.end());
            active_queues = shaper->active_queues.size();
            sum_qi = std::reduce(shaper->active_queues.begin(), shaper->active_queues.end());;
          }
          shaper->magic_packets[0] = 0;
        }
        shaper->last_enqueue_calc_time[0] = now;
        shaper->bytes_in_last_period[0] = 0;
      }
  }
  ...
  for (uint16_t i = 0; i < rx_batch.GetSize(); i++) {
    ...
    if (shaper_queue_id){
      ...
      if(shp_ctx->shapers[shaper_id]->queue_sizes[queue_id] + packet->length() <= shp_ctx->qlen){
        ...
        uint64_t upper_threshold = (shp_ctx->shapers[shaper_id]->bytes_per_ms * 200.0);
        if (active_queues){
          uint16_t sum_qi = std::reduce(shp_ctx->shapers[shaper_id]->active_queues.begin(), shp_ctx->shapers[shaper_id]->active_queues.end());
          if (sum_qi){
            upper_threshold = (shp_ctx->shapers[shaper_id]->bytes_per_ms * 200.0) / sum_qi;
            upper_threshold *= queue_id;
          } else {
            upper_threshold = (shp_ctx->shapers[shaper_id]->bytes_per_ms * 200.0);
          }
        }
        if (shp_ctx->shapers[shaper_id]->bytes_in_last_period[queue_id] > (upper_threshold * 1.5)){
          if (shaper_id > 1024)
            // LOG(INFO)<< shaper_id << " " << queue_id << " " << juggler::time::cycles_to_ms(now - shp_ctx->shapers[shaper_id]->last_enqueue_calc_time[queue_id]) <<" fill queue "<< shp_ctx->shapers[shaper_id]->bytes_in_last_period[queue_id] << " " << shp_ctx->shapers[shaper_id]->upper_threshold * 1.5;
          phantom_dequeue(now, shp_ctx->shapers[shaper_id]);
          ...
          if (shp_ctx->shapers[shaper_id]->bytes_in_last_period[queue_id] < (upper_threshold * 0.5)){
            if (shaper_id > 1024)
              // LOG(INFO)<< shaper_id << " " << queue_id << " " << juggler::time::cycles_to_ms(now - shp_ctx->shapers[shaper_id]->last_enqueue_calc_time[queue_id]) <<" empty the queue "<< shp_ctx->shapers[shaper_id]->bytes_in_last_period[queue_id] << " " << shp_ctx->shapers[shaper_id]->upper_threshold * 0.5;
