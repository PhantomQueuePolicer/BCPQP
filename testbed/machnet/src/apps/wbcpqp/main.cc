...
void phantom_dequeue(uint64_t now, pqp_struct* pqp){
  ...
  // pqp->tokens = std::min(pqp->bytes_per_ms * 1000, pqp->bytes_per_ms * time_elapsed_ms);
  pqp->tokens = pqp->bytes_per_ms * time_elapsed_ms;
  // std::cout<<"dequeuing: "<<pqp->tokens<<std::endl;
  uint16_t active_queues = pqp->active_queues.size();
  uint16_t sum_qi = std::reduce(pqp->active_queues.begin(), pqp->active_queues.end());;
  if (pqp->tokens < (1500 * active_queues)){
    return;
  }
  pqp->last_dequeue_time = now;
  if (pqp->queue_sizes[0] > pqp->tokens/100){
    pqp->queue_sizes[0] -= pqp->tokens/100;
    pqp->dequeues_in_last_period[0] += pqp->tokens/100;
    pqp->tokens -= pqp->tokens/100;
  }
  while (active_queues && sum_qi && pqp->tokens / sum_qi){
    uint64_t per_queue_tokens = pqp->tokens / sum_qi;
    for (uint16_t i=0; i<active_queues; i++){
      ...
      if (pqp->queue_sizes[q_i] > q_i_share){
        pqp->queue_sizes[q_i] -= q_i_share;
        pqp->tokens -= q_i_share;
        pqp->dequeues_in_last_period[q_i] += q_i_share;
      } else {
        ...
        sum_qi = std::reduce(pqp->active_queues.begin(), pqp->active_queues.end());;
      }
      if (juggler::time::cycles_to_ms(now - pqp->last_enqueue_calc_time[q_i]) > 200){
        if (active_queues){ 
          pqp->upper_threshold = (pqp->bytes_per_ms * 200.0) / sum_qi;
          pqp->upper_threshold *= q_i;
        } else {
          pqp->upper_threshold = (pqp->bytes_per_ms * 200.0);
        }
        if (pqp->bytes_in_last_period[q_i] < (pqp->upper_threshold * 0.5)){
          // LOG(INFO)<< "In the dequeue" << q_i << " " << juggler::time::cycles_to_ms(now - pqp->last_enqueue_calc_time[q_i]) <<" empty the queue "<< pqp->bytes_in_last_period[q_i] << " " << pqp->upper_threshold * 0.5;
      ...
      // std::cout<<"after "<< pqp->queue_sizes[q_i]<<std::endl;
    }
  }

  if (pqp->tokens){
    if (pqp->queue_sizes[0] > pqp->tokens){
      pqp->queue_sizes[0] -= pqp->tokens;
      pqp->dequeues_in_last_period[0] += pqp->tokens;
      pqp->tokens -= pqp->tokens;
    } else {
      pqp->tokens -= pqp->queue_sizes[0];
      pqp->dequeues_in_last_period[0] += pqp->queue_sizes[0];
      pqp->queue_sizes[0] = 0;
      pqp->active_queues.erase(std::remove(pqp->active_queues.begin(), pqp->active_queues.end(), 0), pqp->active_queues.end());
    }
    if (juggler::time::cycles_to_ms(now - pqp->last_enqueue_calc_time[0]) > 200){
        pqp->upper_threshold = (pqp->bytes_per_ms * 200.0);
        if (pqp->bytes_in_last_period[0] < (pqp->upper_threshold * 0.5)){
          // LOG(INFO)<< "In the dequeue" << q_i << " " << juggler::time::cycles_to_ms(now - pqp->last_enqueue_calc_time[q_i]) <<" empty the queue "<< pqp->bytes_in_last_period[q_i] << " " << pqp->upper_threshold * 0.5;
          pqp->queue_sizes[0] -= std::min(pqp->queue_sizes[0], pqp->magic_packets[0]);
          if (!pqp->queue_sizes[0]){
            pqp->active_queues.erase(std::remove(pqp->active_queues.begin(), pqp->active_queues.end(), 0), pqp->active_queues.end());
            active_queues = pqp->active_queues.size();
            sum_qi = std::reduce(pqp->active_queues.begin(), pqp->active_queues.end());;
          }
          pqp->magic_packets[0] = 0;
        }
        pqp->last_enqueue_calc_time[0] = now;
        pqp->bytes_in_last_period[0] = 0;
      }
  }
  ...
  for (uint16_t i = 0; i < rx_batch.GetSize(); i++) {
    ...
    if (pqp_queue_id){
      ...
      if(pqp_ctx->pqps[pqp_id]->queue_sizes[queue_id] + packet->length() <= pqp_ctx->qlen){
        ...
        uint64_t upper_threshold = (pqp_ctx->pqps[pqp_id]->bytes_per_ms * 200.0);
        if (active_queues){
          uint16_t sum_qi = std::reduce(pqp_ctx->pqps[pqp_id]->active_queues.begin(), pqp_ctx->pqps[pqp_id]->active_queues.end());
          if (sum_qi){
            upper_threshold = (pqp_ctx->pqps[pqp_id]->bytes_per_ms * 200.0) / sum_qi;
            upper_threshold *= queue_id;
          } else {
            upper_threshold = (pqp_ctx->pqps[pqp_id]->bytes_per_ms * 200.0);
          }
        }
        if (pqp_ctx->pqps[pqp_id]->bytes_in_last_period[queue_id] > (upper_threshold * 1.5)){
          if (pqp_id > 1024)
            // LOG(INFO)<< pqp_id << " " << queue_id << " " << juggler::time::cycles_to_ms(now - pqp_ctx->pqps[pqp_id]->last_enqueue_calc_time[queue_id]) <<" fill queue "<< pqp_ctx->pqps[pqp_id]->bytes_in_last_period[queue_id] << " " << pqp_ctx->pqps[pqp_id]->upper_threshold * 1.5;
          phantom_dequeue(now, pqp_ctx->pqps[pqp_id]);
          ...
          if (pqp_ctx->pqps[pqp_id]->bytes_in_last_period[queue_id] < (upper_threshold * 0.5)){
            if (pqp_id > 1024)
              // LOG(INFO)<< pqp_id << " " << queue_id << " " << juggler::time::cycles_to_ms(now - pqp_ctx->pqps[pqp_id]->last_enqueue_calc_time[queue_id]) <<" empty the queue "<< pqp_ctx->pqps[pqp_id]->bytes_in_last_period[queue_id] << " " << pqp_ctx->pqps[pqp_id]->upper_threshold * 0.5;
