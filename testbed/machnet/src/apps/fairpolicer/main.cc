...
void phantom_dequeue(uint64_t now, fpolicer_struct* fpolicer){
  ...
  while (fpolicer->tokens / active_queues && fpolicer->qsize){
    ...
    for (uint16_t i=0; i<active_queues; i++){
      if (fpolicer->queue_sizes[fpolicer->active_queues[i]] > per_queue_tokens){
        fpolicer->queue_sizes[fpolicer->active_queues[i]] -= per_queue_tokens;
        fpolicer->tokens -= per_queue_tokens;
        fpolicer->qsize -= per_queue_tokens;
      } else {
