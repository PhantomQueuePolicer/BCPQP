...
void phantom_dequeue(uint64_t now, pqp_struct* pqp){
  ...
  // std::cout<<"ph-dequeue "<< time_elapsed_ms <<" "<<pqp->tokens << std::endl;
  if (!pqp->tokens){
    return;
  }
  pqp->last_dequeue_time = now;
  while (pqp->tokens && pqp->qsize){
    ...
    for (uint16_t i=0; i<active_queues; i++){
      if (pqp->queue_sizes[pqp->active_queues[i]] > per_queue_tokens){
        pqp->queue_sizes[pqp->active_queues[i]] -= per_queue_tokens;
        pqp->tokens -= per_queue_tokens;
        pqp->qsize -= per_queue_tokens;
      } else {
        pqp->tokens -= pqp->queue_sizes[pqp->active_queues[i]];
        pqp->qsize -= pqp->queue_sizes[pqp->active_queues[i]];
        pqp->queue_sizes[pqp->active_queues[i]] = 0;
  ...
  for (uint16_t i = 0; i < rx_batch.GetSize(); i++) {
    ...
    if (pqp_queue_id){
      ...
      if(pqp_ctx->pqps[pqp_id]->queue_sizes[queue_id] + packet->length() <= pqp_ctx->qlen){
        ...
        pqp_ctx->pqps[pqp_id]->qsize += packet->length();
